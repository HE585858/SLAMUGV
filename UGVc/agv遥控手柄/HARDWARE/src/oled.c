...
{ 	
	...
	RCC_APB2PeriphClockCmd(OLED_APB2_GPIOB, ENABLE);
	RCC_APB1PeriphClockCmd(OLED_APB1_SPI2, ENABLE);
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(OLED_GPIOB, &GPIO_InitStructure);
	...
	SPI_Init(OLED_SPI2, &SPI_InitStructure); 				/* 根据SPI_InitStruct中指定的参数初始化外设SPIx寄存器 */
 
	SPI_Cmd(OLED_SPI2, ENABLE); /* 使能SPI外设 */
