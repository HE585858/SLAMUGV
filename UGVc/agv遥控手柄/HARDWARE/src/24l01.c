...
static void NRF_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(NRF_APB2_GPIOA|NRF_APB2_GPIOC, ENABLE);
	RCC_APB2PeriphClockCmd(NRF_APB2_SPI1, ENABLE); 

	/* 配置SPI1的SCK(PA5),MISO(PA6),MOSI(PA7)引脚 */ 
	GPIO_InitStructure.GPIO_Pin = SPI_SCK_PIN | SPI_MISO_PIN|SPI_MOSI_PIN; 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_Init(SPI_GPIOA, &GPIO_InitStructure);
	
	/* 配置NRF的CE(PA3),CSN(PA4)引脚 */ 
	GPIO_InitStructure.GPIO_Pin = NRF_CE | NRF_CSN; 
	...
	GPIO_Init(SPI_GPIOC, &GPIO_InitStructure);
